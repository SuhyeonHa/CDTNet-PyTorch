...
class DeconvDecoder(nn.Module):
    ...
    def forward(self, encoder_outputs, image, mask=None):
        ...
        last_feature = self.deconv_blocks[-1](output, mask)


        if self.image_fusion:
            attention_map = torch.sigmoid(3.0 * self.conv_attention(last_feature))
            output = attention_map * image + (1.0 - attention_map) * self.to_rgb(last_feature)
            # F_dec = attention_map * last_feature
        else:
